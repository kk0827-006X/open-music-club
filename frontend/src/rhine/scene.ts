...
export class ArchiveScene {
  ...
  constructor(
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      "三维专辑档案阵列，点击选择，左右拖动切列，上下拖动或滚轮切换列内专辑",
    );
  ...
  async load(assetUrl = publicAsset("assets/archive-cassette.glb")) {
    ...
    for (const texture of this.coverTextures) {
      ...
      this.coverInstances.push(instances);
  ...
  private drawLabel(index: number) {
    ...
    c.fillText("OPEN MUSIC CLUB", 22, 116);
    c.font = "32px MiSans";
    c.fillStyle = "#878476";
    c.fillText("MUSIC ARCHIVE", 25, 174);
    c.fillStyle = "#171713";
  ...
  update(
    ...
    const coverCounts = this.coverInstances.map(() => 0);
    for (const cell of this.cells) {
      ...
      const albumIndex = records[fileAtCell(cell)].albumIndex;
      this.coverInstances[albumIndex].setMatrixAt(coverCounts[albumIndex]++, this.dummy.matrix);
    }
    this.coverInstances.forEach((cover, albumIndex) => {
      cover.count = coverCounts[albumIndex];
